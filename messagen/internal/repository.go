...
func (d *DefinitionRepository) applyTemplatePickers(def *Definition, aliasName AliasName, alias *Alias, state *State) (newTemplates Templates, err error) {
	newDef := *def
	...
	for _, picker := range d.templatePickers {
		...
		newTemplates, err = picker(&newDef, aliasName, alias, state)
		newDef.Templates = newTemplates
		if err != nil {
			return nil, err
		}
...
func generate(def *Definition, aliasName AliasName, alias *Alias, state *State, repo *DefinitionRepository) (chan *State, chan error, error) {
	...
	errChan := make(chan error)
	templates, err := repo.applyTemplatePickers(def, aliasName, alias, state)
	if err != nil {
		return nil, nil, err
	}
